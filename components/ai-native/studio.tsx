  ...
  return (
    <main className={styles.studio} data-night={night} data-ready={ready} data-reduced-motion={reduceMotion} lang={zh ? "zh-CN" : "en"}>
      ...
      <div className={styles.scene} aria-label={zh ? "可交互的三维工作室" : "Interactive 3D studio"}>
        {!failed && <SceneBoundary key={sceneKey} onError={onError}><RoomScene night={night} paused={reduceMotion || agentOpen} selected={selected} focusTarget={selected} onSelect={selectZone} onBlankClick={closePanel} onHover={setHovered} onReady={onReady} onError={onError} onAsk={openAgent} assistantOpen={agentOpen} onHome={onHome} onToggleNight={onToggleNight} locale={locale} /></SceneBoundary>}
      </div>
