...
export function Studio() {
  ...
  const onToggleMotion = useCallback(() => { if (!reduceMotion) setPaused(value => !value); }, [reduceMotion]);
  ...
  const hoverHint = hovered === "assistant" ? (zh ? "我的 AI 分身 · 点击聊聊" : "My AI companion · click to chat")
    ...
    : hovered === "motion" ? (reduceMotion ? (zh ? "已遵循系统减少动态效果设置" : "System reduced motion is enabled") : (zh ? "点击时钟，暂停或继续房间动态" : "Click the clock to pause or resume the room"))
  ...
  return (
    <main className={styles.studio} data-night={night} data-ready={ready} data-reduced-motion={reduceMotion} lang={zh ? "zh-CN" : "en"}>
      ...
      <div className={styles.scene} aria-label={zh ? "可交互的三维工作室" : "Interactive 3D studio"}>
        {!failed && <SceneBoundary key={sceneKey} onError={onError}><RoomScene night={night} paused={isPaused || agentOpen} selected={selected} focusTarget={selected} onSelect={selectZone} onHover={setHovered} onReady={onReady} onError={onError} onAsk={openAgent} assistantOpen={agentOpen} onHome={onHome} onToggleNight={onToggleNight} onToggleMotion={onToggleMotion} locale={locale} /></SceneBoundary>}
      </div>
      ...
      {help && <div className={styles.help}><button type="button" onClick={() => setHelp(false)} aria-label={zh ? "关闭操作说明" : "Close instructions"}><X size={16} /></button><strong>{zh ? "就像在朋友的房间。" : "Make yourself at home."}</strong><p>{zh ? "点人物认识我，点电脑看项目，点书架看教育与科研，点窗户看生活。每天本地时间 08:00–20:00 是白天，点灯可临时切换，下一次早晚交替恢复自动。点时钟暂停动态，墙上标牌返回首页。" : "Meet me by clicking the person. Explore projects on the computer, studies on the shelves, and life through the window. Daylight follows local time, 08:00–20:00. Tap the lamp for a temporary change, the clock to pause, or the exit sign to go home."}</p><p>{zh ? "Tab 选择物品 · Enter 打开 · Esc 返回" : "Tab to choose · Enter to open · Esc to return"}</p></div>}
