            ...
            <Reveal key={item.school} delay={index * 0.08}>
              <article className="panel rounded-[2rem] p-7 md:p-8">
                <div className="mb-5 flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  ...
                  <span className="inline-flex rounded-full border border-white/10 bg-white/5 px-4 py-1.5 text-xs tracking-[0.22em] text-slate-300/84 uppercase">
                    {item.period}
                  </span>
                </div>
        ...
        <Reveal delay={0.16} className="panel rounded-[2rem] p-7 md:p-8">
          ...
          <div className="grid gap-3">
            {educationBadges.map((badge) => (
              <div
                key={badge}
                className="rounded-2xl border border-white/8 bg-white/[0.03] px-4 py-3 text-sm text-slate-200/82"
              >
                {badge}
              </div>
            ))}
